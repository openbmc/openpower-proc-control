...
{
/**
 * Helper method to fetch the value of the specified filed from journal enty
 *
 * @param   jounral - journal entry
 * @param   field - field to search for in the journal entry
 * @return  field value
 */
    ...
    getJournalLog(const std::string& executable, uint16_t maxMessages)
{
    ...
    SD_JOURNAL_FOREACH_BACKWARDS(journal.get())
    {
        ...
        if (syslog == executable)
        {
            ...
            {
                log<level::ERR>(fmt::format("Failed to get timestamp form "
                                            "journal entry ret={}",
                                            ret)
                                    .c_str());
            }
