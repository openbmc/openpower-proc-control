...
void selectBootMaster()
{
    ...
    pdbg_for_each_class_target("proc", procTarget) {
        ...
        if (getBootCount() > 0)
        {
            ...
            if (DT_SET_PROP(ATTR_BACKUP_SEEPROM_SELECT, procTarget, bkpSeePromSelect))
            {
                log<level::ERR>("Attribute [ATTR_BACKUP_SEEPROM_SELECT] set failed");
                openpower::pel::detail::processBootErrorCallback(false);
                throw std::runtime_error("Attribute [ATTR_BACKUP_SEEPROM_SELECT] set failed");
            }
            // To clear trace if success
            openpower::pel::detail::processBootErrorCallback(true);

            bkpMeaSeePromSelect = ENUM_ATTR_BACKUP_MEASUREMENT_SEEPROM_SELECT_PRIMARY;
            if (DT_SET_PROP(ATTR_BACKUP_MEASUREMENT_SEEPROM_SELECT, procTarget, bkpMeaSeePromSelect))
            {
                log<level::ERR>("Attribute [ATTR_BACKUP_MEASUREMENT_SEEPROM_SELECT] set failed");
                openpower::pel::detail::processBootErrorCallback(false);
                throw std::runtime_error("Attribute [ATTR_BACKUP_MEASUREMENT_SEEPROM_SELECT] set failed");
            }
            // To clear trace if success
            openpower::pel::detail::processBootErrorCallback(true);
        }
