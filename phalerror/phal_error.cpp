                                    ...
                                    void* appPrivData)
{
     ...
     * for failure case and for pdbg_target_traverse() case it expected trace
     * because, doing target iteration to get actual ATTR_PHYS_BIN_PATH value.
     * So, Due to this error trace user will get confusion while traces.
 ...
 * "pdbg_target_traverse" api because, doesn't know about respective pdbg target
 * info to get required attribute from it attributes list.
 *
...
/**
 * @brief Used to get PEL expected priority format
 *
...
{
    if (phalPriority == "HIGH")
    {
        return "H";
    }
    else if (phalPriority == "MEDIUM")
    {
        return "M";
    }
    else if (phalPriority == "LOW")
    {
        return "L";
    }
    else if (phalPriority == "NONE")
    {
        return "L";
    }
    else
    {
        log<level::ERR>("Unsupported phal priority used to \
                         get pel priority format",
    ...
    try
    {
        ...
        if (!status)
        {
            ...
            if (ffdc.ffdc_type == HWP)
            {
                ...
                pelAdditionalData.push_back(
                    std::make_pair("HWP_RC", ffdc.hwp_errorinfo.rc));
                pelAdditionalData.push_back(
                    std::make_pair("HWP_RC_DESC", ffdc.hwp_errorinfo.rc_desc));
                ...
                for_each(ffdc.hwp_errorinfo.ffdcs_data.begin(),
                         ...
                         [&pelAdditionalData](
                             ...
                             pelAdditionalData.push_back(
                                 std::make_pair(keyWithPrefix, ele.second));
                         });
                ...
                for_each(
                    ...
                    [&pelAdditionalData, &count,
                     &pelCalloutList](const HWCallout& hwCallout) -> void {
                        ...
                        pelAdditionalData.push_back(std::make_pair(
                        ...
                        {
                            it->second.push_back(jsonCalloutData);
                        }
                    });

                // Adding CDG (callout, deconfigure and gard) targets details
                count = 0;
                for_each(
            ...
            for_each(traceLog.begin(), traceLog.end(),
                     [&pelAdditionalData](
                         std::pair<std::string, std::string>& ele) -> void {
                         pelAdditionalData.push_back(
                             std::make_pair(ele.first, ele.second));
                     });
            ...
            if (it != pelCalloutList.end())
            {
                for_each(it->second.begin(), it->second.end(),
                         [&orderedPelCalloutList](const json& ele) -> void {
                             orderedPelCalloutList.push_back(ele);
                         });
            }
