...
int main(int argc, char** argv)
{
   ...
   {
        rc = open_power::boot::Control::executeStep(
                opt.start_major, opt.start_minor);
        return rc;
   }

   //For range of isteps
   bool completed = false;
   auto begin_step = open_power::boot::Control::majorSteps.find(opt.start_major);
        ...
        iter != open_power::boot::Control::majorSteps.end(); iter++)
   {
        ...
        {
            rc = open_power::boot::Control::executeStep(iter->first, miter->first);
            if (rc < 0)
            {
                return rc;
            }
